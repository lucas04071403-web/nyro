...
import 'package:nyro/core/haptic/haptic_service.dart';
...
import 'package:nyro/features/connection/data/connection_data_providers.dart';
import 'package:nyro/features/connection/data/connection_repository.dart';
...
class ConnectionNotifier extends _$ConnectionNotifier with AppLogger {
  ...
  Future<void> reconnect(ProfileEntity? profile) async {
    if (state case AsyncData(:final value) when value == const Connected()) {
      ...
      final translations = ref.read(translationsProvider).requireValue;
      await ref.read(Preferences.startedByUser.notifier).update(true);
  ...
  Future<void> _disconnect() async {
    ...
    await _connectionRepo.disconnect().mapLeft((err) {
      ...
      dialogNotifier.showCustomAlertFromErr(err.present(translations));
      state = AsyncError(err, StackTrace.current);
    }).run();
  }
}
