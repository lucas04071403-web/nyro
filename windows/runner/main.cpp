                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (SendAppLinkToInstance(L"Nyro"))
  {
    return EXIT_SUCCESS;
  }

  HANDLE hMutexInstance = CreateMutex(NULL, TRUE, L"NyroMutex");
  HWND handle = FindWindowA(NULL, "Nyro");
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS)
  {
    ...
    if (window.SendAppLinkToInstance(L"Nyro"))
    {
      return false;
    }
  ...
  if (!window.Create(L"Nyro", origin, size))
  {
    return EXIT_FAILURE;
  }
